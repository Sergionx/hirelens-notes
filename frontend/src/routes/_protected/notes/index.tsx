...
import { useArchiveNoteMutation } from "@/features/notes/mutations/archive-note-mutation"
...
export const Route = createFileRoute("/_protected/notes/")({
  ...
  loader: ({ context, deps }) => {
    context.queryClient.ensureQueryData(
      getNotesQueryOptions({ archived: false, ...deps.search })
    )
...
function NotesList() {
  ...
  const { data: notes } = useSuspenseQuery(
    getNotesQueryOptions({ archived: false, ...search })
  )
  ...
  return (
    <div className="p-4">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-2xl font-bold">Active Notes</h3>
      ...
      {notes.length === 0 ? (
        <p className="text-muted-foreground">No active notes found.</p>
      ) : (
